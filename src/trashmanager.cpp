...
void TrashManager::moveToTrash(QList<QUrl> urls)
{
    ...
    QProcess::startDetached("long-filemanager", QStringList() << "--move-to-trash" << paths);
}

void TrashManager::emptyTrash()
{
    QProcess::startDetached("long-filemanager", QStringList() << "-e");
}

void TrashManager::openTrash()
{
    QProcess::startDetached("long-filemanager", QStringList() << "trash:///");
}
